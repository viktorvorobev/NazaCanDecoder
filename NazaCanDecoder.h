...
void SendHeartbeat();	// периодически (раз в 2 секунды) шлет heartbeat сообщение контроллеру
void ThreadCanRead();
...
uint16_t GetMotorOutput(int mot);	// возвращает значение подаваемое на мотор (0 - не используется, иначе 16920~35000, 16920 = мотор выкл)
int16_t GetRcIn(int chan);		// возвращает значение получаемое от джойстика для каждого канала (-1000~1000)
...
uint16_t GetBatteryCell(int cell);	// возвращает напряжение банки в мВ
#endif
